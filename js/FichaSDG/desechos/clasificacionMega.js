var $nombreClasificacionMega = $('#clasificacionMegaN');
...
$(document).ready(function() {
    ...
    $btnRealizaraccion.on('click', function() {
        ...
        var op = $(this).val();
        // 
        if ($nombreClasificacionMega.val() != '') {
            swal({ //Mensaje de confirmacion para realizar la accion.
                ...
                if (result.value) {
                    ...
                    $nombreClasificacionMega.parent('div').removeClass('has-error');
                    registrarModificarEstadoClasificacionMega(Number($(this).val()), 2);
                }
            });
        } else {
            if (!($nombreClasificacionMega.parent('div').hasClass('has-error'))) {
                $nombreClasificacionMega.parent('div').addClass('has-error');
            }
        }
    });
...
function registrarModificarEstadoClasificacionMega(idE, estado) {
    $.ajax({
        ...
        data: {
            idEm: String(idE),
            nombre: $nombreClasificacionMega.val(),
            estadoE: String(estado),
            info: 2 //Clasificacion mega
        },
    }).done(function(valor) {
        if (valor == 1) {
...
function enviarInformacionFormulario(info) {
    ...
    $nombreClasificacionMega.val(datos[1]);
}
...
function cambiarEstadoEmpresa(idEmpresa) {
    swal({ //Mensaje de confirmacion para realizar la accion.
        ...
        if (result.value) {
            //Registrar o modificar empresa
            registrarModificarEstadoClasificacionMega(idEmpresa, 1);
            // 
            setTimeout(function() {
                consultarSalario();
            }, 100);
